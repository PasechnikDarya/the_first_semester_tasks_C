...
void tree_dot_print (tree_t *s)
{
	FILE *file_dot = fopen("dif_dot.dot", "w");
	if (file_dot == NULL)
    {
	...
	fprintf(file_dot, "}");
	fclose(file_dot);
	system("dot -Tjpg -o tree.jpg dif_dot.dot");
...
void print_type(node* pos, FILE* dot_out) 
{
    switch (pos->type) 
    {
        ...
        case type_VAR:    fprintf(dot_out, "[label = \"%c\", shape = \"octagon\", "
                                           "color=\"white\", style=\"filled\", "
...
node *node_create (char type, double val, node *lefty, node *righty)
{
	...
	new->left = lefty;
	new->right = righty;
	if (lefty) lefty->parent = new;
	if (righty) righty->parent = new;
